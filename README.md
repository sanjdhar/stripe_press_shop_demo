...
This project uses the Payment Intents API with Stripe Elements:

- `stripe.PaymentIntent.create(...)`: creates a PaymentIntent for the selected book amount in USD.
- `payment_method_types=['card']`: configures the PaymentIntent for card payments in this demo.
- `client_secret`: passed to Stripe.js so the browser can securely render and confirm the Payment Element.
- `stripe.confirmPayment(...)`: confirms the PaymentIntent from the browser and redirects to the success page.
- `stripe.PaymentIntent.retrieve(...)`: retrieves the PaymentIntent on the success page so the app can display the final charge amount, PaymentIntent ID, and payment status.

## Approach

I treated the existing Flask app as the source of truth and added payments at the smallest useful integration points:
...
For a more robust version of the app, I would add:

- A database-backed product catalog and order table.
- A server endpoint that creates PaymentIntents with POST requests instead of creating one on checkout page load.
- Webhook handling for `payment_intent.succeeded` and related events so fulfillment does not depend on the customer returning to the success page.
- Inventory, tax, shipping, and receipt flows.
- Support for additional payment methods by enabling automatic payment methods on the PaymentIntent.
- Authentication or customer records for repeat purchases.
- Automated tests around item selection, PaymentIntent creation, and success page rendering.

## Test Mode Notes

This app should only be run with Stripe test keys during local development. Do not commit your `.env` file or live API keys.
